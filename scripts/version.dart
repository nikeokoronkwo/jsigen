...
void syncVersion([bool pkl = false]) async {
  ...
  var path = 'js/package.json';
  final pkgJson = File(path).readAsStringSync();
  final pkgjsonEditor = YamlEditor(pkgJson);
  pkgjsonEditor.update(['version'], version);
  await File(path).writeAsString(pkgjsonEditor.toString().replaceAll(version, '"$version"'));
